...
async def code(interaction: discord.Interaction, difficulty: Choice[int], id: int):
    ...
    if os.path.exists(file_path):
        ...
        embed_main.add_field(name=text("cmd.code.submit"), value=text("cmd.code.submit_description",diffToName(difficulty.value),id))
        ...
        if not os.path.exists(f"assets/code_test/{difficulty.value}/{id+1}.json"):
            embed_main.set_author(name=f"{diffToName(difficulty.value)} #{id} {text('cmd.code.last')}")
        else:
        ...
        await interaction.response.send_message(text("cmd.code.not_exist",diffToName(difficulty.value),id), ephemeral=True)
...
async def submit_code(interaction: discord.Interaction, difficulty: Choice[int], id: int, language: Choice[str]):
    ...
    if os.path.exists(file_path):
       ...
       await interaction.response.send_modal(SubmitWindow())
        ...
        await interaction.response.send_message(text("cmd.code_submit.not_exist",diffToName(difficulty.value),id), ephemeral=True)
